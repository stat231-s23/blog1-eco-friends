...
Our data is from Kaggle and was created for the purpose of analyzing the health of different fast food items. The full data set can be found [HERE](https://www.kaggle.com/datasets/ulrikthygepedersen/fastfood-nutrition), where you can view other variables that were not a part of our analysis. Information for the map in the _Conclusion_ can be found [HERE.](https://www.kaggle.com/datasets/khushishahh/fast-food-restaurants-across-us)
...
kable(arb_summaries) %>%
  ...
  row_spec(3, color = "#27adad") %>%
  row_spec(4, color = "#C77CFF") 
...
# Putting names of items back into our cluster data set
...
sub <- ggplot(subway_c, aes(x = calories, y = sugar, label = item)) + 
  ...
  labs(title = "Subway Items Grouped By on Calories and Sugar Content",
       ...
       y = "Sugar (grams)",
       color = "Cluster") +
 # Choose theme and make adjustments
 theme_economist() +
 theme(plot.title = element_text(size=12, colour = "#2E5984"),
       axis.text = element_text(color = "#2E5984"),
       text = element_text(color = "#2E5984"))

# Output our visualization  
ggplotly(sub)
```
<center>*Click on the points to learn more about the menu items or click on the legend to isolate the clusters.*</center>
...
# Get information about items in clusters 
subway_summaries <- tidy(subway_ck)%>%
    rename(Calories = calories, Sugar = sugar, 
           Size = size, Withiness = withinss)


# Output table with summary statistics, colored by cluster.
kable(subway_summaries) %>%
  row_spec(1, color = "#f54242") %>%
  row_spec(2, color = "#2abd09") %>%
  row_spec(3, color = "#42A7F5") 

# Split our clusters by item
clusterlist_subway <- split(subway$item, subway_ck$cluster)

# Making data frame for visualization
subway_df <- matrix(NA
...
# Output table with items, colored by cluster.
subway_df %>%
...
# Standardize all our numeric variables (subtract mean and divide by SD)
mutate(across(where(is.numeric), ~scale(.)[,1], .names = "{.col}_scaled")) %>%
...
# Make some visualizations, colored by cluster.
...
The map below depicts only a fraction of the true amount of locations for the eight previously mentioned fast food chains in the United States. In reality, there are over nearly 65,000 combined locations for these eight franchises alone, and the map depicts only 5,995. This makes it abundantly clear that no matter which state you live in, it’s likely that you’ll come across at least one of these popular restaurant chains. 

```{r, echo=FALSE}
# Generate Map
mapview(restmap, xcol = "longitude", ycol = "latitude", crs = 4269, grid = FALSE, zcol = "name", legend = TRUE,
layer.name = 'Restaurant')
```
<center>*Click a point on the map to learn more about the restaurant located there.*</center>

As time goes by, more restaurants are introducing more veggie-forward meals like salads and bowls to provide customers with the perfect combination of health and affordability. After reading through our blog, we hope that you are now equipped with all the information you need to keep your health goals in mind while enjoying your meal!
...
- Chiu, Allyson. “Is Subway’s Bread Actually Bread? The Dispute Is a Reminder of How Much Sugar Lurks in Our Food.” The Washington Post, 8 Oct. 2020, www.washingtonpost.com/lifestyle/wellness/bread-subway-sugar-nutrition/2020/10/08/807cc1da-04fc-11eb-a2db-417cddf4816a_story.html.
  
- “Cholesterol.” Centers for Disease Control and Prevention, 20 Mar. 2023, www.cdc.gov/cholesterol/index.htm.   
  
- C. Sievert. Interactive Web-Based Data Visualization with R, plotly, and
  shiny. Chapman and Hall/CRC Florida, 2020.
  
- Cut down on Added Sugars - Health, health.gov/sites/default/files/2019-10/DGA_Cut-Down-On-Added-Sugars.pdf. Accessed 9 May 2023. 

- “Fast Food Effects: Short-Term, Long-Term, Physical, Mental, and More.” Medical News Today, www.medicalnewstoday.com/articles/324847. Accessed 9 May 2023. 
  
- “Health and Wellness Banner Transparent PNG - 2298X624 - Free Download on Nicepng.” NicePNG.Com, www.nicepng.com/ourpic/u2e6y3r5i1t4e6u2_health-and-wellness-banner/. Accessed 9 May 2023. 
...
- Staff, Restaurant Business. “The Largest Mexican Chains in the U.S.” Restaurant Business, 13 May 2020, www.restaurantbusinessonline.com/financing/largest-mexican-chains-us. 

- “The Sweet Danger of Sugar.” Harvard Health, 6 Jan. 2022, www.health.harvard.edu/heart-health/the-sweet-danger-of-sugar#:~:text=%22The%20effects%20of%20added%20sugar,%2C%22%20says%20Dr.%20Hu. 
